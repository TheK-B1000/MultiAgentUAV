...
| `v6i26_lro_actor_step_ablation` (aliases `v6i26_actor_step`, `v6i26_actor_step_ablation`) | `v6i26_latent_response_oracle` | `DIAGNOSTIC` | Optimizer-control ablation only: separate z-actor/critic grad clipping + `latent_lro_z_actor_lr_mult=2.0` (critic LR unchanged). Locked OP9/`z0` margin surface. Primary movement gate is fixed-batch init→final KL ≥1e-3 (authority-probe protocol); training `approx_kl` is secondary; `clip_fraction>0` is **not** required. Contract: `actor_step_ablation_contract.json`. |
