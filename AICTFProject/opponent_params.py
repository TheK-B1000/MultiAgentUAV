...
def sample_opponent_params(
    ...
    if kind == "SCRIPTED":
        if key == "OP1":
            ...
            attack_sync_window = 0
            noise_sigma = 0.0
        elif key in ("OP2", "OP2_EASY"):
            deception_prob = rng.uniform(0.0, 0.15)
            coordinated_attack = False
            attack_sync_window = 0
            noise_sigma = rng.uniform(0.0, 0.05)
        elif key in ("OP3", "OP3_EASY", "OP3_HARD", "OP3_ADAPTIVE", "OP3_SYNC", "OP3_EVASIVE"):
            deception_prob = rng.uniform(0.1, 0.35)
            coordinated_attack = rng.random() < 0.5 if key == "OP3" else (key == "OP3_SYNC")
