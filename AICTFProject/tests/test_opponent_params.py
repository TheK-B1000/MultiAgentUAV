...
class OpponentParamsTests(unittest.TestCase):
    ...
    def test_op6_immediate_dual_rush_style_for_4v4(self) -> None:
        params = sample_batched_opponent_params(
            kind="SCRIPTED",
            key="OP6",
            phase="OP6",
        ...
        self.assertTrue(torch.isfinite(params["speed_mult"]).all().item())
        self.assertTrue((params["attacker_style"] == 1).all().item())
        self.assertTrue((params["defender_style"] == 0).all().item())
        self.assertTrue(torch.allclose(params["role_switch_prob"], torch.full((64,), 0.04)))
        self.assertTrue(((params["role_switch_prob"] >= 0.0) & (params["role_switch_prob"] <= 0.90)).all().item())

    def test_op7_deep_fortress_no_longer_samples_switcher_mixture(self) -> None:
        params = sample_batched_opponent_params(
            kind="SCRIPTED",
            key="OP7",
            phase="OP7",
        ...
        self.assertTrue((params["defender_style"] == 1).all().item())
        self.assertTrue(torch.allclose(params["role_switch_prob"], torch.full((4096,), 0.03)))
        self.assertGreaterEqual(float(params["speed_mult"].min().item()), 0.86)
        self.assertLessEqual(float(params["speed_mult"].max().item()), 1.00)
