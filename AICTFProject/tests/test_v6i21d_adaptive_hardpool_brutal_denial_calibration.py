"""Pinning tests for V6I21D brutal adaptive hardpool denial calibration preset."""
...
class V6i21dAdaptiveHardpoolBrutalDenialPresetTests(unittest.TestCase):
    def test_aliases_resolve_to_brutal_denial_arm(self) -> None:
        aliases = [
            "v6i21d",
            "v6i21d_adaptive_hardpool_brutal_denial_calibration",
            "v6i21d_adaptive_hardpool_brutal_denial",
            "latent_v6i21d_adaptive_hardpool_brutal_denial_calibration",
    ...
    def test_v6i21d_is_exact_identity_diff_over_v6i21c(self) -> None:
        parent_cfg = apply_preset(PPOConfig(), "v6i21c")
        normalize_and_validate_training_config(parent_cfg)
        parent = dataclasses.asdict(parent_cfg)
        cfg_obj = apply_preset(PPOConfig(), "v6i21d")
        ...
        self.assertEqual(cfg_obj.experiment_id, "v6i21d")
        self.assertEqual(cfg_obj.run_tag, "v6i21d_adaptive_hardpool_brutal_denial_calibration")

    def test_v6i21d_preserves_v6i21_scaffold_and_op_pool(self) -> None:
        cfg = apply_preset(PPOConfig(), "v6i21d")
