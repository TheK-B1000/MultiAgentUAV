"""One-shot held-out payoff EVAL for the frozen H-OG-PSP V4 terminal.

The V4 trajectory-identity result was frozen before this evaluator. This file
carries the V3 six-cell payoff design and paired-bootstrap gate unchanged.

Run: python experiments/eval_hog_psp_v4.py --device cuda
"""
...
SPEC = SD / "sppo" / "HOG_PSP_V4_EVAL_SPEC.json"
FROZEN = SD / "sppo" / "HOG_PSP_V4_MODEL_FROZEN.json"
MECHANISM = SD / "sppo" / "HOG_PSP_V4_MECHANISM_DIAGNOSTIC.json"
OUT = SD / "sppo" / "HOG_PSP_V4_EVAL_RESULT.json"
ROWS_CSV = SD / "sppo" / "hog_psp_v4_eval_rows.csv"
...
def _preflight() -> tuple[dict, dict, dict, Path]:
    ...
    if frozen["TERMINAL_RECORD_VALIDITY"]["verdict"].split()[0] != "VALID":
        raise SystemExit("REFUSING: the V4 training run was not established VALID")
    if frozen["EVAL_STATE_AT_FREEZE"]["touched"]:
        raise SystemExit("REFUSING: the V4 EVAL block was not untouched at model freeze")
    if mechanism["READING"] != "TRAJECTORY_IDENTITY_PARTIAL":
        raise SystemExit("REFUSING: frozen V4 mechanism reading does not match eval context")
    if mechanism["PRESERVATION_LABEL"] != "IDENTITY_DEGRADED":
        raise SystemExit("REFUSING: frozen V4 preservation label does not match eval context")
    if not mechanism["INTEGRITY_CHECKS_ALL_PASSED"]:
        raise SystemExit("REFUSING: V4 mechanism integrity did not pass")
    if mechanism["V4_EVAL_touched"]:
        raise SystemExit("REFUSING: mechanism record says the V4 EVAL block was touched")
    ...
    if actual != expected or actual != spec["MODEL_UNDER_TEST"]["sha256"]:
        raise SystemExit("REFUSING: terminal checkpoint identity mismatch")
    if OUT.is_file() or ROWS_CSV.is_file():
        raise SystemExit("REFUSING: an H-OG-PSP V4 EVAL output already exists; EVAL is one-shot")
    if spec["SEEDS"]["block"] != "11400101..11400132":
        raise SystemExit("REFUSING: eval seed block drifted from the frozen spec")
    if [EVAL_SEEDS[0], EVAL_SEEDS[-1]] != [11_400_101, 11_400_132]:
...
def main() -> int:
    ...
    print(f"H-OG-PSP V4 HELD-OUT PAYOFF EVAL  {_now()}")
    ...
    print("  trajectory identity is frozen PARTIAL / DEGRADED; this run measures payoff only\n",
          flush=True)
    ...
    if not getattr(student.model.critic, "private_z_heads", False):
        raise SystemExit("REFUSING: loaded V4 critic does not have private latent heads")
    ...
    def run_cell(policy, pole: str, z: int | None, seed: int) -> dict:
        ...
        try:
            ...
            assert_live_opponent_batch(core, genomes, allowed_keys=(key,),
                                       context=f"hog v4 eval {pole} z{z} seed {seed}")
            terminal = None
    ...
    OUT.write_text(json.dumps({
        "record": "H-OG-PSP V4 held-out payoff EVAL",
        "status": "FROZEN_RESULT",
        "one_shot": True,
        "utc": _now(),
        "implements": "HOG_PSP_V4_EVAL_SPEC.json",
        "VERDICT": verdict,
        "trajectory_identity_frozen_before_eval": mechanism["READING"],
        "identity_preservation_frozen_before_eval": mechanism["PRESERVATION_LABEL"],
        ...
        "bootstrap": {"procedure": "paired percentile bootstrap over seed-level episode outcomes",
                      ...
                      "unit": "evaluation seed", "provenance": "unchanged from V3"},
