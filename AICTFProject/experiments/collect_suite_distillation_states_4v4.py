"""Collect SUITE_DISTILLATION_4V4 under CLOSEST_DEFENDS(k=2).
...
Implements SUITE_DISTILLATION_4V4_SPEC.json. One-shot: refuses if the dataset
manifest already exists.

Run:
  python experiments/collect_suite_distillation_states_4v4.py --device cuda
  python experiments/collect_suite_distillation_states_4v4.py --device cpu --smoke
"""
...
SPEC_PATH = SD / "SUITE_DISTILLATION_4V4_SPEC.json"
OUT_DIR = SD / "suite_distillation_4v4" / "states"
MANIFEST = SD / "SUITE_DISTILLATION_4V4_DATASET.json"
N_AGENTS = 4
K_DEFEND = 2
...
def main() -> int:
    ...
    if smoke:
        ...
        out_dir = SD / "suite_distillation_4v4_SMOKE" / "states"
        manifest = SD / "SUITE_DISTILLATION_4V4_DATASET_SMOKE.json"
    else:
    ...
    print(f"SUITE DISTILLATION 4V4 COLLECT  {_now()}  device={device}"
        ...
        for ep_i, seed in ep_iter:
            ...
            try:
                ...
                assert_live_opponent_batch(
                    ...
                    context=f"suite distill 4v4 {pole} seed {seed}",
                )
    ...
    manifest.write_text(json.dumps({
        "record": "Suite distillation state set (4v4)"
                  + (" -- NON-SCIENTIFIC SMOKE" if smoke else ""),
        "status": "SMOKE_NOT_SCIENTIFIC" if smoke else "FROZEN_DATASET",
        "utc": _now(),
        "implements": "SUITE_DISTILLATION_4V4_SPEC.json#DATASET",
        "team_size": N_AGENTS,
        "allocator": {
