"""Forced-z crossover eval for suite 4v4 distilled sharing arms.
...
Implements SUITE_SHARING_4V4_CROSSOVER_EVAL_SPEC.json.

Run:
  python experiments/eval_suite_sharing_crossover_4v4.py --arm fully_shared --dry-run
  python experiments/eval_suite_sharing_crossover_4v4.py --arm share_encoder --device cuda
  python experiments/eval_suite_sharing_crossover_4v4.py --arm share_backbone --device cuda
  python experiments/eval_suite_sharing_crossover_4v4.py --arm share_macro --device cuda
"""
...
def main() -> int:
    ...
    LOG = SD / "suite_sharing" / "4v4" / args.arm / "crossover_eval.log"
    ...
    print(f"SUITE 4V4 CROSSOVER EVAL  {label}  {_now()}  device={device}")
    ...
    if int(obs_space.spaces["grid"].shape[0]) != N_AGENTS:
        raise SystemExit("FAIL-CLOSED: env agent dim != 4")
    probe.close()
    ...
    if not needs_entity:
        raise SystemExit("REFUSING: suite 4v4 students are entity-repair; refusing non-entity eval")
