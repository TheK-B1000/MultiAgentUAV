"""Reward V3 reliability probe -- objective-dominant budget, 300k steps, fresh seed.
...
Gate: artifacts/reward_v3/PROBE_GATE_FROZEN.json (frozen first).

Run:  python experiments/run_reward_v3_probe.py --seed 2900001
"""
...
V3_SEEDS = (2_900_001, 2_900_002, 2_900_003)
ABLATION_SEEDS = V3_SEEDS
TOTAL_TIMESTEPS = 300_000
CHECKPOINT_INTERVAL = 50_000
COLLAPSE_ZONE = 225_000
SPARSE_TAG_NO_FLAG_POINTS_ABLATED = 0.0
V3_ACTION_FAILED_PUNISHMENT = -0.004   # derived: 0.0181 x baseline, from 184 events/episode
...
def run_tag_for(seed: int) -> str:
    return f"g0_v2_rewardv3_seed{seed}"


def artifact_dir_for(seed: int) -> Path:
    return PROJECT_ROOT / "artifacts" / "reward_v3" / run_tag_for(seed)
...
def build_config(seed: int):
    ...
    cfg.env_action_failed_punishment = V3_ACTION_FAILED_PUNISHMENT
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"REWARD V3 RELIABILITY PROBE  seed={seed}  steps={TOTAL_TIMESTEPS:,}")
    print(f"sparse_tag_no_flag_points:   {BASELINE_SPARSE_TAG_POINTS} -> {SPARSE_TAG_NO_FLAG_POINTS_ABLATED}")
    print(f"action_failed_punishment:    {BASELINE_ACTION_FAILED_PUNISHMENT} -> {V3_ACTION_FAILED_PUNISHMENT}")
    ...
    report = {
        "probe": "Reward V3 reliability probe",
        "seed": seed,
