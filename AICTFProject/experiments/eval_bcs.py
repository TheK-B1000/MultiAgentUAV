"""Balanced-causal-sampling sealed EVAL: the one-shot held-out payoff test for both arms.

Implements BALANCED_CAUSAL_SAMPLING_SPEC.json#EVAL_PROTOCOL against both terminal checkpoints
frozen in BCS_MODELS_FROZEN.json. Structurally identical to eval_trunk_freeze.py/eval_sac_rft.py
-- same _mean_ci bootstrap, same PRIMARY_GATE, same tie/reversal-triggers-an-audit-first rule.

Run:  python experiments/eval_bcs.py --device cuda
"""
...
FROZEN = SD / "BCS_MODELS_FROZEN.json"
OUT = SD / "BCS_EVAL_RESULT.json"
ROWS_CSV = SD / "bcs_eval_rows.csv"
PREAUDIT_FLAG = SD / "BCS_EVAL_INTEGRITY_REQUIRED.json"

EVAL_SEEDS = list(range(11_902_001, 11_902_065))
...
def _preflight():
    ...
    if spec["status"] != "FROZEN_BEFORE_IMPLEMENTATION":
        raise SystemExit(f"REFUSING: BCS spec not frozen: {spec['status']!r}")
    ...
    if OUT.is_file() or ROWS_CSV.is_file() or PREAUDIT_FLAG.is_file():
        raise SystemExit("REFUSING: a BCS EVAL output already exists; one-shot")
...
def main() -> int:
    ...
    print(f"BCS SEALED EVAL  {_now()}")
    print("  both terminal checkpoints sha256 VERIFIED against BCS_MODELS_FROZEN.json")
    ...
    def run_cell(policy, pole: str, z: int, seed: int) -> dict:
        ...
        try:
            ...
            assert_live_opponent_batch(core, genomes, allowed_keys=(key,),
                                       context=f"bcs eval {pole} z{z} seed {seed}")
            terminal = None
    ...
    if tie_or_reversal:
        PREAUDIT_FLAG.write_text(json.dumps({
            "record": "BCS EVAL integrity audit REQUIRED", "status": "FLAGGED",
            "utc": _now(), "implements": "BALANCED_CAUSAL_SAMPLING_SPEC.json#EVAL_PROTOCOL.tie_or_reversal",
            ...
            "rule": "requires a row-level integrity audit, written before its rows are read. "
                    ...
                    "BCS_EVAL_RESULT.json is written until that audit exists.",
    ...
    table = {(True, False): "balanced sampling recovered crossover where unbalanced sampling "
                            "did not -- supports the supervision-imbalance hypothesis",
             (True, True): "crossover recovered under both; balancing not necessary under "
                           "this run",
             (False, False): "balanced sampling also insufficient -- the bottleneck is more "
                             "likely segment diversity than sampling weight, or the erosion "
                             "mechanism is unrelated to the causal bank at all"}
    ...
    OUT.write_text(json.dumps({
        "record": "Balanced-causal-sampling sealed EVAL", "status": "FROZEN_RESULT",
        "one_shot": True, "utc": _now(), "implements": "BALANCED_CAUSAL_SAMPLING_SPEC.json#EVAL_PROTOCOL",
        "seeds": {"block": [EVAL_SEEDS[0], EVAL_SEEDS[-1]], "n": len(EVAL_SEEDS),
                  "shared_across_arms": True},
        "PRIMARY_GATE": {
            "BALANCED_TREATMENT": {"delta_A": delta["A_T"], "delta_B": delta["B_T"],
                                   "passes": treatment_pass},
            "UNBALANCED_CONTROL": {"delta_A": delta["A_C"], "delta_B": delta["B_C"],
                                   "passes": control_pass}},
