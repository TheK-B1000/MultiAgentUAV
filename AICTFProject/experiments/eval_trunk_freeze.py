"""Trunk-freeze sealed EVAL: the one-shot held-out payoff test for both arms.

Implements TRUNK_FREEZE_SPEC.json#EVAL_PROTOCOL against both terminal checkpoints frozen in
TRUNK_FREEZE_MODELS_FROZEN.json. Structurally identical to eval_rscft.py -- same _mean_ci
bootstrap, same PRIMARY_GATE, same tie/reversal-triggers-an-audit-first rule -- because the
gate itself did not change between experiments; only the treatment under test did.

One addition this experiment's own spec calls for: SECONDARY_DIAGNOSTIC_NOT_A_GATE. Even if
the full crossover gate fails, delta_A alone recovering a positive sign with LCB95 > 0 would
directly confirm the erosion-mechanism hypothesis and is reported descriptively regardless of
the primary gate's outcome.

Run:  python experiments/eval_trunk_freeze.py --device cuda
"""
...
ROWS_CSV = SD / "trunk_freeze_eval_rows.csv"
PREAUDIT_FLAG = SD / "TRUNK_FREEZE_EVAL_INTEGRITY_REQUIRED.json"
...
def _preflight():
    ...
    if spec["status"] != "FROZEN_BEFORE_IMPLEMENTATION":
        raise SystemExit(f"REFUSING: trunk freeze spec not frozen: {spec['status']!r}")
    ...
    if OUT.is_file() or ROWS_CSV.is_file() or PREAUDIT_FLAG.is_file():
        raise SystemExit("REFUSING: a trunk-freeze EVAL output already exists; one-shot")
...
def main() -> int:
    ...
    print(f"TRUNK-FREEZE SEALED EVAL  {_now()}")
    print("  both terminal checkpoints sha256 VERIFIED against TRUNK_FREEZE_MODELS_FROZEN.json")
    ...
    def run_cell(policy, pole: str, z: int, seed: int) -> dict:
        ...
        try:
            ...
            assert_live_opponent_batch(core, genomes, allowed_keys=(key,),
                                       context=f"trunk-freeze eval {pole} z{z} seed {seed}")
            terminal = None
    ...
    if tie_or_reversal:
        PREAUDIT_FLAG.write_text(json.dumps({
            "record": "Trunk-freeze EVAL integrity audit REQUIRED", "status": "FLAGGED",
            "utc": _now(), "implements": "TRUNK_FREEZE_SPEC.json#EVAL_PROTOCOL.tie_or_reversal",
            ...
            "rule": "requires a row-level integrity audit, written before its rows are read. "
                    ...
                    "TRUNK_FREEZE_EVAL_RESULT.json is written until that audit exists.",
    ...
    if unmatched:
        reading = (f"UNMATCHED COMBINATION (treatment="
                   ...
                   f"control={'PASS' if control_pass else 'FAIL'}). Recorded as such rather "
                   "than coerced into the nearest row; requires an explicit PI reading.")

    # secondary diagnostic, per spec -- not a gate, reported regardless of primary outcome
    secondary = {arm: {"delta_A_positive_and_significant":
                       bool(delta[f"A_{arm[0]}"]["mean"] > 0 and delta[f"A_{arm[0]}"]["lcb95"] > 0)}
                for arm in ARMS}
    ...
    print(f"\n  SECONDARY DIAGNOSTIC (not a gate): delta_A alone positive+significant?")
    for arm in ARMS:
        print(f"    {arm:9s} {secondary[arm]['delta_A_positive_and_significant']}")
    print(f"\n  READING: {reading}")

    OUT.write_text(json.dumps({
        "record": "Trunk-freeze sealed EVAL", "status": "FROZEN_RESULT", "one_shot": True,
        "utc": _now(), "implements": "TRUNK_FREEZE_SPEC.json#EVAL_PROTOCOL",
        "seeds": {"block": [EVAL_SEEDS[0], EVAL_SEEDS[-1]], "n": len(EVAL_SEEDS),
                  "shared_across_arms": True},
        "PRIMARY_GATE": {
            "TREATMENT_trunk_frozen_plus_causal": {"delta_A": delta["A_T"], "delta_B": delta["B_T"],
                                                   "passes": treatment_pass},
            "CONTROL_trunk_frozen_only": {"delta_A": delta["A_C"], "delta_B": delta["B_C"],
                                          "passes": control_pass}},
        "ATTRIBUTION": {"Gamma_A": gamma_a, "Gamma_B": gamma_b, "passes": gamma_pass},
        "SECONDARY_DIAGNOSTIC_NOT_A_GATE": secondary,
        "READING": reading, "unmatched_interpretation_combination": unmatched,
