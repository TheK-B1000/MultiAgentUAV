#!/usr/bin/env python3
"""GATE 2C -- does a defender DELAY the enemy while costing us tempo?
...
AGENTS = 2
SEED_BASE = 2_000_001                   # fresh block
...
def main() -> int:
    ...
    p.add_argument("--out-dir", default="artifacts/gate2c_first_capture_v2")
    args = p.parse_args()
    ...
    emit("=" * 84)
    emit(f"GATE 2C -- first-capture latency under {RULESET_ID}, {MAP}, vs {args.opponent}")
    ...
    for pr in problems:
        emit(f"    {pr}")
    if not ok:
        emit("\nGATE 2C: ABORT -- integrity failure; refusing to analyze")
        (out / "gate2c_report.txt").write_text("\n".join(lines))
        return 1
    ...
    sup = ["blue_captured", "red_captured", "blue_score", "red_score",
           ...
           "mean_num_attackers", "episode_steps"]
    ...
    summary = {
        "gate": "gate2c_first_capture_latency",
    ...
    (out / "manifest.json").write_text(json.dumps({
        "experiment": "gate2c_first_capture_latency", "ruleset_id": RULESET_ID, **RULESET,
        "map": MAP, "resolved_map": RESOLVED_MAP, "opponent": args.opponent,
    ...
    crit = ["experiments/gate2c_first_capture_latency_v2.py",
