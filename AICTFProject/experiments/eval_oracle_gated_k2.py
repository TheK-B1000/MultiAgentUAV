"""One-shot held-out EVAL for the oracle-gated K=2 run. Implements the frozen spec.

Scores the cross-evaluation matrix on seeds never used for training, rehearsal, or
calibration:

    V(z0, A)   V(z1, A)        Pole A = OP6 + SDS2_A_payoff_INIT_3 overlay
    V(z0, B)   V(z1, B)        Pole B = OP7, no overlay

Crossover gate, adopted UNCHANGED from eval_exp2_k2_terminal.py so the criterion
cannot be accused of having been chosen for this result:
...
Run:  python experiments/eval_oracle_gated_k2.py --device cuda
"""
...
SPEC = SD / "sppo" / "ORACLE_GATED_K2_EVAL_SPEC.json"
FROZEN = SD / "sppo" / "ORACLE_GATED_K2_MODEL_FROZEN.json"
OUT = SD / "sppo" / "ORACLE_GATED_K2_EVAL_RESULT.json"
ROWS_CSV = SD / "sppo" / "oracle_gated_k2_eval_rows.csv"

EVAL_SEEDS = list(range(10_700_129, 10_700_161))
...
def _preflight() -> tuple[dict, dict, Path]:
    ...
    if spec["SEEDS"]["block"] != "10700129..10700160":
...
def main() -> int:
    ...
    print(f"ORACLE-GATED K=2 HELD-OUT EVAL  {_now()}")
    print(f"  checkpoint sha256 VERIFIED against the freeze record")
    ...
    def run_cell(policy, pole: str, z: int | None, seed: int) -> dict:
        ...
        Latent forcing uses the SAME mechanism as the frozen EXP2 terminal eval --
        fixed_latent_strategy / fixed_latent_strategy_id then reset_strategy(). An
        earlier draft guessed a set_forced_latent() that does not exist, which would
        have run z0 and z1 as the SAME policy and produced a meaningless crossover.
    ...
    verdict = ("ORACLE_GATED_K2_CROSSOVER_CONFIRMED" if passed
               else "ORACLE_GATED_K2_CROSSOVER_NOT_CONFIRMED")
    ...
    OUT.write_text(json.dumps({
        "record": "Oracle-gated K=2 held-out EVAL",
        "status": "FROZEN_RESULT", "one_shot": True, "utc": _now(),
        "implements": "ORACLE_GATED_K2_EVAL_SPEC.json (frozen c82c1ef5)",
        "VERDICT": verdict,
        ...
        "interpretive_context_frozen_before_the_result": {
            "replay_factor": 188.86,
            "budget": "1,003,520 steps vs EXP2C's 2,002,944 -- comparisons carry a ~2x gap",
            "thin_margin": "76.4% of rehearsal states had |Delta| = 1"},
