"""G0-v2 sparse tag-reward ablation -- one factor, 300k steps, fresh seed.

Tests the leading suspect causally: with the sparse tag reward removed, do
...
ONLY ``sparse_tag_no_flag_points`` differs from the frozen G0-v2 configuration.
...
Gate: artifacts/g0_v2_tagreward_ablation/PROBE_GATE_FROZEN.json (frozen first).

Run:  python experiments/run_g0_v2_tagreward_ablation.py --seed 2700001
"""
...
ABLATION_SEEDS = (2_700_001, 2_700_002, 2_700_003)
TOTAL_TIMESTEPS = 300_000
CHECKPOINT_INTERVAL = 50_000
COLLAPSE_ZONE = 225_000
SPARSE_TAG_NO_FLAG_POINTS_ABLATED = 0.0
BASELINE_ACTION_FAILED_PUNISHMENT = -0.2   # restored: not the factor under test
BASELINE_SPARSE_TAG_POINTS = 100.0
DEVICE = "cuda"


def run_tag_for(seed: int) -> str:
    return f"g0_v2_notag_seed{seed}"


def artifact_dir_for(seed: int) -> Path:
    return PROJECT_ROOT / "artifacts" / "g0_v2_tagreward_ablation" / run_tag_for(seed)
...
def build_config(seed: int):
    ...
    cfg.env_action_failed_punishment = BASELINE_ACTION_FAILED_PUNISHMENT
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"G0-v2 TAG-REWARD ABLATION  seed={seed}  steps={TOTAL_TIMESTEPS:,}")
    print(f"sparse_tag_no_flag_points: {BASELINE_SPARSE_TAG_POINTS} (baseline) -> "
          f"{SPARSE_TAG_NO_FLAG_POINTS_ABLATED} (ablated)  [ONLY changed factor]")
    ...
    report = {
        "probe": "G0-v2 sparse tag-reward ablation",
        "seed": seed,
        ...
        "only_changed_factor": "sparse_tag_no_flag_points",
