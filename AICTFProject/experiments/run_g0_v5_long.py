"""G0-V5 LONG RUN — 1,000,000 steps under the frozen, verified-aligned Reward V5 -- objective-dominant budget, 300k steps, fresh seed.
...
Gate: artifacts/g0_v5_long/G0_V5_LONG_RUN_SPEC.json (frozen first).

Run:  python experiments/run_g0_v5_long.py --seed 3200001
"""
...
V4_OWN_OOB_POINTS = -70.0
V5_MINE_TAG_POINTS = 0.0   # measured 0.129-0.182/ep in V4 = 15-20% of terminal   # derived: 0.0181 x baseline, from 184 events/episode
...
def artifact_dir_for(seed: int) -> Path:
    return PROJECT_ROOT / "artifacts" / "g0_v5_long" / run_tag_for(seed)
...
def build_config(seed: int):
    ...
    cfg.env_sparse_mine_tag_points = V5_MINE_TAG_POINTS
...
def _reward_mass(art: Path, horizon: int = 240) -> dict:
    ...
    early = {k: seg(c, 0, 100_000) * horizon for k, c in cols.items()}
    late = {k: seg(c, 900_000, 1_010_000) * horizon for k, c in cols.items()}
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"G0-V5 LONG RUN  seed={seed}  steps={TOTAL_TIMESTEPS:,}")
    ...
    print(f"sparse_mine_tag_points:      100.0 -> {V5_MINE_TAG_POINTS}  (budgeted from measured V4 rate)")
    ...
    report = {
        ...
        "sparse_mine_tag_points": V5_MINE_TAG_POINTS,
