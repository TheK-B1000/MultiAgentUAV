"""G0-v2 ALL-TAG-REWARD ablation -- one factor, 300k steps, fresh seed.
...
Gate: artifacts/g0_v2_alltag_ablation/PROBE_GATE_FROZEN.json (frozen first).

Run:  python experiments/run_g0_v2_alltag_ablation.py --seed 2800001
"""
...
ABLATION_SEEDS = (2_800_001, 2_800_002, 2_800_003)
...
def run_tag_for(seed: int) -> str:
    return f"g0_v2_alltag0_seed{seed}"


def artifact_dir_for(seed: int) -> Path:
    return PROJECT_ROOT / "artifacts" / "g0_v2_alltag_ablation" / run_tag_for(seed)
...
def main() -> int:
    ...
    print("=" * 78)
    print(f"G0-v2 ALL-TAG-REWARD ABLATION  seed={seed}  steps={TOTAL_TIMESTEPS:,}")
    ...
    print("[ALL tag rewards zero; captures still rewarded, OOB still penalised]")
    ...
    report = {
        "probe": "G0-v2 all-tag-reward ablation",
        "seed": seed,
        ...
        "only_changed_factor": "tag reward family (no-flag + carrier)",
