...
def preflight(policy: str) -> dict:
    if not PROTOCOL.is_file() or not SEMANTICS.is_file():
        raise SystemExit("REFUSING: SAPPO freeze or semantics amendment missing")
    if not COMPAT.is_file():
        raise SystemExit("REFUSING: live compatibility check has not been run")
...
def build_argv(spec: dict) -> list[str]:
    ...
    return [
        ...
        "--sappo-anchor-dataset", str(DEMO / spec["anchor"]),
        "--sappo-anchor-lambda", str(LAMBDA_ANCHOR),
        "--sappo-anchor-cadence", str(CADENCE),
    ]
...
def main() -> int:
    ...
    print(f"  additional steps {ADDITIONAL_STEPS:,} (cumulative target ~1,500,000)")
    ...
    for pol in todo:
        ...
        (run_dir / "r2b_manifest.json").write_text(json.dumps({
            ...
            "only_variable_changed": "teacher rehearsal (lambda=0.10, cadence 1:4)",
            "anchor_dataset": spec["anchor"],
