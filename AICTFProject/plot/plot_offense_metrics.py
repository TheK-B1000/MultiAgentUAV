...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=str, default=None, help="Output directory for plots (default: AICTFProject/figures)")
    parser.add_argument("--table", type=str, default=None, help="Write summary table to this CSV (default under AICTFProject/csv)")
    ...
    default_csv_dir = project_root / "csv"
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else default_figures
    if args.table:
        t = Path(args.table)
        if len(t.parts) <= 1:
            args.table = str(default_csv_dir / t.name)
