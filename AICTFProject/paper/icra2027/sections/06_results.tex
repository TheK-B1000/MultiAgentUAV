...
\label{sec:results}
%======================================================================
% PAPER SCALE NARRATIVE -- LOCKED 2026-09-23 (PI):
%   2v2 --> 4v4 --> repaired 6v6
%
% INCLUDE:
%   - 2v2 specialists + sharing ladder (+ generalist context as already drafted)
%   - 4v4 split crossover PASS (CLOSEST_DEFENDS + frozen ATTACK + learned pi_D)
%   - repaired/mature 6v6 split once school-PC pipeline seals (ONLY 6v6 main-story result)
%   - fully shared strategy-conditioned baseline AFTER repaired 6v6 (then include)
%
% EXCLUDE from final narrative (keep artifacts for provenance only):
%   - old 6v6 Share-0 / Share-Encoder compression / Rung-1 distillation crossover
%     (sealed FAIL on disk; do not typeset as the paper's 6v6 result)
%
% Reverses draft note of 2026-09-11 that excluded 4v4 from this section.
% PENDING: repaired 6v6 sealed Delta_A / Delta_B (search PENDING-REPAIRED-6V6).
%======================================================================
...
% ---------------------------------------------------------------- PENDING-REPAIRED-6V6
% LOCKED 2026-09-23: the paper's only 6v6 main-story result is the repaired/mature
% split (school-PC pipeline: entity-repair A/B -> freeze ATTACK -> pi_D +200k + N'
% teacher -> CLOSEST_DEFENDS(k=1) -> sealed crossover). Do NOT fill this slot from
% RUNG1_6V6_CROSSOVER_EVAL_RESULT.json / old Share-Encoder compression (EXCLUDED).
% After school-PC seals, typeset Delta_A / Delta_B from that confirmatory artifact.
% ----------------------------------------------------------------------------
\noindent
\textbf{[PENDING-REPAIRED-6V6 --- school-PC mature split not yet sealed; do not typeset.]}
\[
...
\noindent
The interpretation is pre-committed and does not depend on the outcome. This
$6$v$6$ measurement uses the same algorithmic recipe as the sealed $4$v$4$ split
(mature specialists, frozen ATTACK, learned defender under heuristic role
allocation), with scale-specific composition $k{=}1$. If the criterion is
satisfied, complementary specialization under that recipe transfers from four to
six agents. If it is not, the split result is scale-bounded under an
apples-to-apples foundation and must be reported as such. Either outcome is
scored against the same $\Delta_A$/$\Delta_B$ gate used at $2$v$2$ and $4$v$4$.
Legacy $6$v$6$ Share-0 / Share-Encoder compression evaluations are archived and
