...
class GameField:
    def __init__(self, grid: List[List[int]]):
        ...
        self.policies: Dict[str, Policy] = {
            "blue": HeuristicPolicy(),         # baseline for blue
        ...
        for friendly_team, enemy_team in (
            ...
            for agent in friendly_team:
                if not agent.isEnabled():
                    # If they just respawned this frame, isEnabled() will be True
                    continue

                # 1. Hazards at current pos (mines & suppression can emit rewards via GameManager)
                ...
                # Internal heuristic opponents / scripted baseline
                if self.use_internal_policies and not side_external and cooldown <= 0.0:
                    self.decide(agent)
                    self.decision_cooldown_seconds_by_agent[agent_key] = (
                        self.decision_interval_seconds
                    )
                else:
                    # External control (e.g., RL for BLUE) – macro-actions are set outside
                    # and all rewards are generated by GameManager's event-driven logic.
                    pass

                # 3. Distance-to-home shaping for flag carriers (BLUE-focused)
                self.manager.reward_flag_carry_progress(agent)

                # 4. Pickups & flag logic at new pos (can also emit reward events)
    ...
    def decide(self, agent: Agent) -> None:
        ...
        if isinstance(policy, Policy):
            ...
            # Fallback: treat as a callable(agent, game_field) -> MacroAction
