...
from rl.custom_ppo.rollout.transition_builder import (
    flag_territory_features_changed,
    obs_rows_from_next_step,
)
...
class RolloutCollector:
    ...
    def flag_territory_features_changed(
        ...
        return flag_territory_features_changed(pre, post, eps=eps)
    ...
    def obs_rows_from_next(
        ...
        return obs_rows_from_next_step(next_obs, infos)
    ...
    def z_for_bootstrap(
        ...
        fixed_z = fixed_latent_bootstrap_z(hparams, z_t)
    ...
    def next_values(
        ...
        device = self.device
        gs = torch.as_tensor(
            global_state_rows_from_step_infos(infos, next_global_state),
            dtype=torch.float32,
            device=device,
        )
        with torch.no_grad():
    ...
    def on_episode_done(
        ...
        *,
        ...
        runtime = self.runtime
        bs, rs = episode_scores_from_info(info)
    ...
    def _compose_step_rewards(
        ...
        return compose_step_rewards(
            infos,
            device=self.device,
            hparams=self.hparams,
            shaping_coef=float(self._reward_shaping_coef_fn()),
            router_reward_enabled=bool(getattr(self.cfg, "router_reward_enabled", False)),
        )
