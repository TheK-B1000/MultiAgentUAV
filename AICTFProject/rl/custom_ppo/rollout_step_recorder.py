...
class StepFrame:
    ...
    Constructed inside :meth:`RolloutCollector._step_once` and passed to
    :meth:`RolloutStepRecorder.record`. Latent-only fields default to
    ``None`` and are required only when ``trainer.use_latent_strategy``
    is ``True``.
    """

    # State observed at the decision point (pre-step).
    obs: Dict[str, np.ndarray]
    context_state: torch.Tensor
    decision_global_state_np: np.ndarray

    # Policy / critic outputs for the chosen action.
    ...
    # Composed reward components (already passed through
    # ``_compose_training_reward_components``).
    reward_component: Dict[str, torch.Tensor]

    # Episode flags / opponent label, all aligned to ``num_envs``.
    terminated: np.ndarray
    truncated: np.ndarray
    opp_row: torch.Tensor
    infos: List[Dict[str, Any]]

    # Latent-only fields. Populated only when the trainer is running in
    # latent-strategy mode; raise inside the recorder if they're missing.
