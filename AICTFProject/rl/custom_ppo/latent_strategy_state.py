...
class LatentStrategyState:
    ...
    def __init__(self, trainer: "CustomPPOTrainer") -> None:
        ...
        self.macro_open_ctx = torch.zeros(
            (n_envs, int(trainer.model.global_state_dim)), dtype=torch.float32, device=device
        )
        self.macro_open_z = torch.zeros((n_envs,), dtype=torch.long, device=device)
        self.macro_open_log_prob = torch.zeros((n_envs,), dtype=torch.float32, device=device)
        self.macro_return_accum = torch.zeros((n_envs,), dtype=torch.float32, device=device)
        self.macro_steps_accum = torch.zeros((n_envs,), dtype=torch.long, device=device)
        self.macro_has_open = torch.zeros((n_envs,), dtype=torch.bool, device=device)
        self.rollout_strategy_macro_records: list[dict[str, Any]] = []
    ...
    def macro_accumulate_step(self, rewards: torch.Tensor) -> None:
        ...
        active = self.macro_has_open
        if not bool(active.any().item()):
            return
        self.macro_return_accum = torch.where(
            active, self.macro_return_accum + r.to(self.macro_return_accum), self.macro_return_accum
        )
        self.macro_steps_accum = torch.where(
            active, self.macro_steps_accum + 1, self.macro_steps_accum
        )

    def macro_finalize(self, finalize_mask: torch.Tensor, *, reason: str = "boundary") -> int:
        if not self._v6i1_macro_enabled():
            return 0
        eligible = finalize_mask & self.macro_has_open
        ...
        for env_i in idx.detach().cpu().tolist():
            env_i = int(env_i)
            steps = int(self.macro_steps_accum[env_i].detach().cpu().item())
            macro_return = float(self.macro_return_accum[env_i].detach().cpu().item())
            self.rollout_macro_finalized_count += 1
            self.rollout_macro_length_sum += steps
            self.rollout_macro_return_sum += macro_return
            self.macro_return_running_count += 1
            alpha = 1.0 / float(self.macro_return_running_count)
            self.macro_return_running_mean += alpha * (macro_return - self.macro_return_running_mean)
            if steps < 1:
    ...
    def macro_open(
        ...
        *,
        ...
        target_dim = int(self.macro_open_ctx.shape[1])
    ...
    def apply_macro_strategy_ppo(self) -> dict[str, float]:
        ...
        trainer = self.trainer
        stats = self.empty_macro_strategy_stats()
        stats["latent_macro_finalized_count"] = float(self.rollout_macro_finalized_count)
        stats["latent_macro_dropped_short_count"] = float(self.rollout_macro_dropped_short_count)
        if self.rollout_macro_finalized_count > 0:
            stats["latent_macro_mean_length"] = float(
                self.rollout_macro_length_sum / self.rollout_macro_finalized_count
            )
            stats["latent_macro_mean_return"] = float(
                self.rollout_macro_return_sum / self.rollout_macro_finalized_count
            )
        if not self._v6i1_macro_enabled() or trainer.fixed_latent_strategy:
            return stats
        records = list(self.rollout_strategy_macro_records)
        if not records:
            return stats
        device = trainer.device
        states = torch.stack(
        ...
        for _ in range(n_epochs):
            ...
            if trainer.model.episode_strategy_value_head is not None:
                v_loss = 0.5 * (macro_returns - v_z).pow(2).mean()
            else:
        ...
        stats["latent_macro_advantage_mean"] = float(sum(adv_means) / n)
        stats["latent_macro_advantage_std"] = float(sum(adv_stds) / n)
        stats["latent_macro_policy_loss"] = float(sum(pg_losses) / n)
        stats["latent_macro_value_loss"] = float(sum(value_losses) / n)
        stats["latent_macro_clipfrac"] = float(sum(clipfracs) / n)
        stats["latent_macro_approx_kl"] = float(sum(kls) / n)
        if grad_norms:
            stats["latent_macro_grad_norm"] = float(sum(grad_norms) / len(grad_norms))
    ...
    def strategy_for_step(
        ...
        if bool(episode_start_mask.any().item()):
            ...
            self.v6i1_episode_rehearsal[start_idx] = False
            ...
            if self.selector_hidden is not None:
                self.selector_hidden[start_idx] = 0.0
            ...
            if is_v6i1_staged_trainer(trainer):
                ...
                if phase in ("B", "C") and rehearsal_prob > 0.0:
                    ...
                    if bool(rehearsal_local.any().item()):
                        ...
                        self.v6i1_episode_rehearsal[rehearsal_idx] = True
                        ...
                        self.episode_forced_z[rehearsal_idx] = True
                        self.episode_forced_z_id[rehearsal_idx] = forced_z
                router_idx = start_idx[~self.v6i1_episode_rehearsal[start_idx]]
        ...
        if bool(resample_mask.any().item()):
            ...
            if is_v6i1_staged_trainer(trainer):
                ...
                if epsilon > 0.0:
                    router_resample = resample_mask & (~self.v6i1_episode_rehearsal)
        ...
        if bool(macro_boundary_mask.any().item()):
            self.macro_finalize(macro_boundary_mask, reason="boundary")
            self.macro_open(
                macro_boundary_mask,
                global_state=global_state,
                z_idx=z_idx,
        ...
        if bool((resample_mask | (forced_active & episode_start_mask)).any().item()):
            if self.selector_hidden is not None:
                _, h_new = trainer.model._forward_q_phi(global_state, self.selector_hidden)
                self.selector_hidden = h_new.detach()
