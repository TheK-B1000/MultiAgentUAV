...
def build_team_obs(
    ...
    *,
    ...
    """
    Build team observation dict: grid, vec, optional mask, optional agent_mask.
    ...
    Args:
        ...
        n_macros: Number of macro actions (5).
        n_targets: Number of targets; if None, use getattr(gf, "num_macro_targets", 8).
        role_macro_mask_fn: Optional (agent_index, macro_mask) -> macro_mask (e.g. role mask).
        vec_append_fn: Optional (base_vec) -> extended_vec (e.g. high-level mode dims).

    Returns:
        Dict with "grid", "vec", optionally "mask", optionally "agent_mask" (when tokenized).
    """
...
__all__ = ["TeamObs", "build_team_obs"]
