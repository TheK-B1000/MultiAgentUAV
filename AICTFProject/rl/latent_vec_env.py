...
Call ``attach_strategy_encoder(encoder)`` after the policy is built so sampling uses q_φ(z|s).
"""
...
class LatentStrategyVecEnvWrapper(VecEnvWrapper):
    ...
    def _sample_z_indices(self, idx: np.ndarray) -> None:
        core = self._core()
        if core is None:
    ...
    def reset(self) -> dict[str, np.ndarray]:
        ...
        self._sample_z_indices(np.arange(self.num_envs, dtype=np.int64))
            ...
            if isinstance(infos[i], dict):
                ...
                if isinstance(to, dict):
                    ...
                    if isinstance(gs_t, np.ndarray) and gs_t.ndim == 1:
                        ...
                        gs_t = None
                    infos[i]["terminal_observation"] = self._augment_terminal_row(row, int(old_z[i]), old_logits[i], gs_row=gs_t)

        self._steps_since_reset += 1
        if d.any():
            self._steps_since_reset[d] = 0
            self._sample_z_indices(np.where(d)[0].astype(np.int64))
        elif self.resample_every_n > 0:
            mask = (self._steps_since_reset % self.resample_every_n) == 0
            if np.any(mask):
                self._sample_z_indices(np.where(mask)[0].astype(np.int64))
