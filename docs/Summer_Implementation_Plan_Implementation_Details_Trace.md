...
| **Must-have baselines** | `experiments/phase6_experiment_matrix.py` now names the required controls explicitly: `flat_ppo_marl` (`--no-latent-strategy`), `latent_no_persistence` (`--latent-resample-every 20 --latent-lam-p 0.0`), and `fixed_latent` (`--fixed-latent-strategy --fixed-latent-id 0`). The old invalid `k2` row was replaced by `k6` because the implementation validates $K \in \{4,6\}$. |
