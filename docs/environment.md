...
where `dense_weight=0.25` and `sparse_weight=1.0` by default. Defensive PBRS uses `pbrs_defense_coef=0.5` so denial/protection is present without drowning out event throughput. The active GPU reward knobs are normalized through `RewardConfig` / `RewardProfile`, while `GPUFieldConfig` keeps flat fields for legacy callers. The PPO collector mirrors this same bounded reward formula after applying any trainer-side shaping decay, so `dense_weight`, `reward_scale`, `reward_clip`, and stalemate penalties are part of the actual training target. Training logs expose the components separately as `reward_terminal`, `reward_offense`, `reward_pbrs`, `reward_team`, `reward_sparse`, `reward_sparse_points`, `reward_failure`, and `reward_total`, plus audit aggregates `reward_outcome_mean`, `reward_shaping_mean`, and `reward_failure_to_outcome_abs`.
