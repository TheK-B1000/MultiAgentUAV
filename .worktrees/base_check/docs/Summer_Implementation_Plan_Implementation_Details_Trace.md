...
| $\mathcal{L} = \mathcal{L}_{\text{MARL}} + \lambda_p \mathcal{L}_{\text{persist}} - \lambda_H \mathcal{H}(z)$. | <span style="color:#1e8449">**Form:** PPO loss + `latent_lam_p * persist_loss` + an entropy bonus inside `latent_loss` in `rl/custom_ppo.py` `update()`. The canonical v5i6 row uses `latent_entropy_mode = "marginal"` and applies the bonus to the batch-marginal distribution $E_s[q_\phi(z|s)]$; v5i4/v5i5 preserve the older mean conditional strategy entropy as comparison rows. (Action-entropy and strategy-entropy are separate terms.) **Sign:** the trainer **minimizes** a scalar with the entropy term arranged as a bonus, so higher entropy is preferred when `latent_lam_h > 0`.</span> |
